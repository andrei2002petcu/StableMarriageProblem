...
(define (match person engagements pref1 pref2 queue)
  (let loop ((p-list (get-pref-list pref1 person)))
    (if (null? p-list)
        ...
        (let* ((next-pers (car p-list)) (next-pers-partner (get-partner engagements next-pers)))
          (cond
            ((member next-pers queue) (loop (cdr p-list)))
            ((equal? next-pers-partner #f) (update-engagements engagements next-pers person))
            ((preferable? (get-pref-list pref2 next-pers) person next-pers-partner)
             (match next-pers-partner (update-engagements engagements next-pers person) pref1 pref2 queue))
...
(define (path-to-stability engagements mpref wpref queue)
  (let loop ((queue queue) (engagements engagements))
    (if (null? queue)
        engagements
        (let ((person (car queue)) (wfirst engagements) (mfirst (reverse-engagements engagements)))
          (if (member person (get-men mpref))
              (loop (cdr queue) (match person wfirst mpref wpref queue))
              (loop (cdr queue) (reverse-engagements (match person mfirst wpref mpref queue))))))))

(define (reverse-engagements engagements)
  (foldl (lambda (x L) (cons (cons (cdr x) (car x)) L)) '() engagements))
...
(define (update-stable-match engagements mpref wpref)
  (let ((unstable-couples (get-unstable-couples engagements mpref wpref)))
    (path-to-stability (filter (lambda (x) (if (member x unstable-couples) #f #t)) engagements) mpref wpref (get-couple-members unstable-couples))))
...
(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream)
      empty-stream
      (let* ((first-pref-pair (stream-first pref-stream)) (first-SMP (gale-shapley (car first-pref-pair) (cdr first-pref-pair))))
        (stream-cons first-SMP (helper first-SMP (stream-rest pref-stream))))))

(define (helper prev-SMP pref-stream)
  (if (stream-empty? pref-stream)
        empty-stream
        (let* ((pref-pair (stream-first pref-stream)) (next-SMP (update-stable-match prev-SMP (car pref-pair) (cdr pref-pair))))
          (stream-cons next-SMP (helper next-SMP (stream-rest pref-stream))))))
