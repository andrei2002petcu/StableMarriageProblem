...
(define (get-women wpref)
  (foldr (lambda (x L) (cons (car x) L)) '() wpref))
...
(define (get-pref-list pref person)
  (foldl (lambda (x L) (if (equal? (car x) person) (append (cdr x) L) L)) '() pref))
...
(define (get-partner engagements person)
  (define x (find-first (lambda (pair) (equal? person (car pair))) engagements))
  (if (equal? x #f)
      x
      (cdr x)))
...
(define (change-first p L val)
  (if (null? L)
      '()
      (if (p (car L))
          (cons val (change-first (lambda (x) #f) (cdr L) val))
